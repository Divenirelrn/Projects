...
// MTCNN::MTCNN(){}
MTCNN::MTCNN(ZipWrapper *zip_wrapper) {

  std::vector<char> pnet_param = zip_wrapper->ReadFileBin("det1.param");
  std::vector<char> pnet_bin = zip_wrapper->ReadFileBin("det1.bin");
  Pnet.load_param_mem((const char *)pnet_param.data());
  Pnet.load_model((const unsigned char *)pnet_bin.data());

  std::vector<char> rnet_param = zip_wrapper->ReadFileBin("det2.param");
  std::vector<char> rnet_bin = zip_wrapper->ReadFileBin("det2.bin");

  Rnet.load_param_mem((const char *)rnet_param.data());
  Rnet.load_model((const unsigned char *)rnet_bin.data());

  std::vector<char> onet_param = zip_wrapper->ReadFileBin("det3.param");
  std::vector<char> onet_bin = zip_wrapper->ReadFileBin("det3.bin");

  Onet.load_param_mem((const char *)onet_param.data());
  Onet.load_model((const unsigned char *)onet_bin.data());
}

MTCNN::MTCNN(const std::vector<std::string> param_files,
...
void MTCNN::generateBbox(ncnn::Mat score, ncnn::Mat location,
...
void MTCNN::nmsTwoBoxs(vector<Bbox> &boundingBox_, vector<Bbox> &previousBox_,
  ...
  for (std::vector<Bbox>::iterator ity = previousBox_.begin();
       ity != previousBox_.end(); ity++) {
    for (std::vector<Bbox>::iterator itx = boundingBox_.begin();
      ...
      if (!modelname.compare("Union"))
        ...
        IOU = IOU / ((boundingBox_.at(i).area < previousBox_.at(j).area)
                         ? boundingBox_.at(i).area
                         : previousBox_.at(j).area);
      }
      if (IOU > overlap_threshold &&
          boundingBox_.at(i).score > previousBox_.at(j).score) {
        // if (IOU > overlap_threshold) {
...
void MTCNN::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold,
                string modelname) {
  if (boundingBox_.empty()) {
    return;
  }
  ...
  while (vScores.size() > 0) {
    ...
    vPick[nPick] = last;
    nPick += 1;
    for (std::multimap<float, int>::iterator it = vScores.begin();
         it != vScores.end();) {
      ...
      if (!modelname.compare("Union"))
        IOU = IOU /
              (boundingBox_.at(it_idx).area + boundingBox_.at(last).area - IOU);
      else if (!modelname.compare("Min")) {
        IOU = IOU / ((boundingBox_.at(it_idx).area < boundingBox_.at(last).area)
                         ? boundingBox_.at(it_idx).area
...
void MTCNN::refine(vector<Bbox> &vecBbox, const int &height, const int &width,
                   bool square) {
  if (vecBbox.empty()) {
...
void MTCNN::extractMaxFace(vector<Bbox> &boundingBox_) {
  ...
  for (std::vector<Bbox>::iterator itx = boundingBox_.begin() + 1;
       itx != boundingBox_.end();) {
    itx = boundingBox_.erase(itx);
  }
}
...
void MTCNN::PNet() {
  ...
  for (size_t i = 0; i < scales_.size(); i++) {
    ...
    nms(boundingBox_, nms_threshold[0]);
    firstBbox_.insert(firstBbox_.end(), boundingBox_.begin(),
                      boundingBox_.end());
    boundingBox_.clear();
  }
}
void MTCNN::RNet() {
  secondBbox_.clear();
  int count = 0;
  for (vector<Bbox>::iterator it = firstBbox_.begin(); it != firstBbox_.end();
       it++) {
    ncnn::Mat tempIm;
    copy_cut_border(img, tempIm, (*it).y1, img_h - (*it).y2, (*it).x1,
                    img_w - (*it).x2);
    ncnn::Mat in;
    ...
    if ((float)score[1] > threshold[1]) {
      for (int channel = 0; channel < 4; channel++) {
        it->regreCoord[channel] =
            (float)bbox[channel]; //*(bbox.data+channel*bbox.cstep);
      }
      it->area = (it->x2 - it->x1) * (it->y2 - it->y1);
...
void MTCNN::ONet() {
  thirdBbox_.clear();
  for (vector<Bbox>::iterator it = secondBbox_.begin(); it != secondBbox_.end();
       it++) {
    ncnn::Mat tempIm;
    copy_cut_border(img, tempIm, (*it).y1, img_h - (*it).y2, (*it).x1,
                    img_w - (*it).x2);
...
//    ncnn::Mat::PIXEL_BGR2RGB, img.cols, img.rows); std::vector<Bbox>
//    finalBbox; detect(ncnn_img, finalBbox); const int num_box =
//    finalBbox.size(); rectangles.resize(num_box); for(int i = 0; i < num_box;
//    i++){
//        rectangles[i] = cv::Rect(finalBbox[i].x1, finalBbox[i].y1,
//        finalBbox[i].x2 - finalBbox[i].x1 + 1, finalBbox[i].y2 -
//        finalBbox[i].y1 + 1);
//    }
//}
