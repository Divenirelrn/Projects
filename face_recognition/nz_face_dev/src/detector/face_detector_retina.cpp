...
Detector::Detector()
    : _nms(0.4), _threshold(0.6), _mean_val{104.f, 117.f, 123.f},
      _retinaface(false), Net(new ncnn::Net()) {}
...
void Detector::Init(const std::string &model_param,
                    const std::string &model_bin) {
  int ret = Net->load_param(model_param.c_str());
  ret = Net->load_model(model_bin.c_str());
}

void Detector::Init(ZipWrapper *wrapper) {
  std::vector<char> net_param = wrapper->ReadFileBin("retinaface_opt.param");
  std::vector<char> net_bin = wrapper->ReadFileBin("retinaface_opt.bin");
  Net->load_param_mem((const char *)net_param.data());
  Net->load_model((const unsigned char *)net_bin.data());
}

void Detector::Detect(const cv::Mat &bgr, std::vector<bbox> &boxes) {
  // Timer timer;
  // timer.tic();
  ncnn::Mat in = ncnn::Mat::from_pixels_resize(
      bgr.data, ncnn::Mat::PIXEL_BGR, bgr.cols, bgr.rows, bgr.cols, bgr.rows);
...
//        result.y2 = in.h;
      ...
      for (int j = 0; j < 5; ++j) {
        result.point[j]._x =
            (tmp.cx + *(landms + (j << 1)) * 0.1 * tmp.sx) * in.w;
        result.point[j]._y =
            (tmp.cy + *(landms + (j << 1) + 1) * 0.1 * tmp.sy) * in.h;
      }
...
void Detector::create_anchor_retinaface(std::vector<box> &anchor, int w,
                                        int h) {
  //    anchor.reserve(num_boxes);
  anchor.clear();
