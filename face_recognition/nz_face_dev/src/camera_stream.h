...
public:
  ...
  cv::Mat GetAffineRGBImage(const cv::Mat &affine_matrix, const int width_out, const int height_out) const {
    int sw     = width_;
    int sh     = height_;
    int rot_sw = sw;
    int rot_sh = sh;
    ...
    std::shared_ptr<MNN::Tensor> tensor(MNN::Tensor::create<uint8_t>(std::vector<int>{1, height_out, width_out, 3}, img_out.data));
...
int GetPreviewSize() {
  ...
  cv::Mat GetScaledImage(const float scale, bool with_rotation) {
    ...
    if (rotation_mode_ == ROTATION_270 && with_rotation) {
      ...
      std::shared_ptr<MNN::Tensor> tensor(MNN::Tensor::create<uint8_t>(std::vector<int>{1, scaled_height, scaled_width, 3}, img_out.data));
      process->convert(buffer_, sw, sh, 0, tensor.get());
      return img_out;
