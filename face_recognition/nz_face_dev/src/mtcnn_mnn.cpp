...
// MTCNN::MTCNN(){}
MTCNN_MNN::MTCNN_MNN(ZipWrapper *zip_wrapper) {
  //PnetInfer.reset(new ModelInfer(zip_wrapper, "det1.mnn", 1, mean_vals, norms_vals));
  //PnetInfer->Init("data", "strided_slice", 112, 112);

  RnetInfer.reset(new ModelInfer(zip_wrapper, "det2.mnn", 1, mean_vals, norm_vals));

  std::vector<std::string> output_name;
  output_name.push_back("prob1");
  RnetInfer->Init("data", output_name, 24, 24);

  //OnetInfer.reset(new ModelInfer(zip_wrapper, "det3-half.mnn", 1, mean_vals, norms_vals));
  //OnetInfer->Init("data", "strided_slice", 112, 112);
}

MTCNN_MNN::~MTCNN_MNN() {
}

void MTCNN_MNN::SetMinFace(int minSize) { minsize = minSize; }
