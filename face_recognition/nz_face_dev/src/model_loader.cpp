//
// Created by Jack Yu on 26/12/2017.
//
...
void xor_encryption(char *data, int length, const unsigned char *key,
                    int key_length) {
...
ModelLoader::ModelLoader(const std::string &model_path) { Reset(model_path); }
...
void ModelLoader::Reset(const std::string &model_path) {
  ...
  if (magic_number != 1128) {
    std::cout << "error can't find magic number ,magic number 1128"
              << std::endl;
    exit(0);
  ...
  for (int i = 0; i < number_of_models; i++) {
    ...
    xor_encryption(model[i].prototxtBuffer, modelSize[i].prototxt_size, key,
                   256);
    file.read(model[i].caffemodelBuffer, modelSize[i].caffemodel_size);
    xor_encryption(model[i].caffemodelBuffer, modelSize[i].caffemodel_size, key,
                   256);
  }
...
std::size_t ModelLoader::Count() const { return n2i_.size(); }

Model *ModelLoader::ReadModel(int idx) const {
  assert(idx < number_of_models);
  return &model[idx];
}
Model *ModelLoader::ReadModel(const std::string &name) const {
  int idx = n2i_.at(name);
  return &model[idx];
}
} // namespace pr
