...
public:
  MTCNN(ZipWrapper *wrapper);
  MTCNN(const std::vector<std::string> param_files,
...
private:
  void generateBbox(ncnn::Mat score, ncnn::Mat location,
  ...
  void refine(vector<Bbox> &vecBbox, const int &height, const int &width,
              bool square);
