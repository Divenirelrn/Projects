...
int main(int argc, char **argv) {
  ...
  const string N = argv[4];
  int NN = atoi(N.c_str());
  std::cout << "NN:" << NN << std::endl;
  for (int k = 0; k < NN; k++) {
    Delay(2000);
    i++;
    std::cout << "try detect image batch P" << i * 5 << std::endl;
  }
  ...
  //  vector<string> mtcnn_param_list{MTCNN_PARAM_1, MTCNN_PARAM_2,
  //  MTCNN_PARAM_3};
