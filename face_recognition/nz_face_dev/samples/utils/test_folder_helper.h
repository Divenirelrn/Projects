...
readFileListSuffix_jpy(const char *basePath, std::vector<std::string> suffix, std::vector<std::string>& file_names, bool recursive) {
    DIR *dir;
    struct dirent *ptr;
    char base[1000];
...
//            printf("d_name:%s/%s\n", basePath, ptr->d_name);
            ...
            for (int i=0; i < suffix.size(); i++) {
            	real = decide_ext(ptr->d_name, suffix[i].c_str());
                if (real) {
                    std::stringstream ss;
...
//            printf("d_name:%s/%s\n", basePath, ptr->d_name);
        } else if (ptr->d_type == 4 and recursive)    ///dir
        {
            ...
            //readFileListSuffix_jpy(base, suffix, file_names, recursive);
        }
    }
    closedir(dir);
    return file_list;
